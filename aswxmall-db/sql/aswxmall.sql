...
CREATE TABLE `aswxmall_goods` (
  ...
  `freight_price` decimal(10,2) DEFAULT '-1.00' COMMENT '邮费，-1采用默认计费/其他值为单独邮费',
...
CREATE TABLE `aswxmall_groupon` (
  ...
  `rules_id` int(11) NOT NULL COMMENT '团购规则ID，关联aswxmall_groupon_rules表ID字段',
  `user_id` int(11) NOT NULL COMMENT '用户ID',
...
CREATE TABLE `aswxmall_order` (
  ...
  `revise_freight_price` decimal(10,2) DEFAULT '0.00' COMMENT '管理员减免邮寄费用',
  ...
  `revise_price` decimal(10,2) DEFAULT '0.00' COMMENT '管理员减免费用',
  `order_price` decimal(10,2) NOT NULL COMMENT '订单费用， = goods_price + freight_price - coupon_price',
  `actual_price` decimal(10,2) NOT NULL COMMENT '实付费用， = order_price - integral_price - revise_price - revise_freight_price',
  `revise_price_admin_id` int(11) DEFAULT NULL COMMENT '修改价格管理员Id',
