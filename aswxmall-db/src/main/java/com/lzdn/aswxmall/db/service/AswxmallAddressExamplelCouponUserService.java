...
@Service
public class AswxmallAddressExamplelCouponUserService {
    @Resource
    private AswxmallCouponUserMapper couponUserMapper;

    public Integer countCoupon(Integer couponId) {
        AswxmallCouponUserExample example = new AswxmallCouponUserExample();
    ...
    public void add(AswxmallCouponUser couponUser) {
        couponUser.setAddTime(LocalDateTime.now());
        couponUser.setUpdateTime(LocalDateTime.now());
        couponUserMapper.insertSelective(couponUser);
    }

    public List<AswxmallCouponUser> queryList(Integer userId, Integer couponId, Short status, Integer page, Integer size, String sort, String order) {
        AswxmallCouponUserExample example = new AswxmallCouponUserExample();
        AswxmallCouponUserExample.Criteria criteria = example.createCriteria();
        if (userId != null) {
    ...
    public List<AswxmallCouponUser> queryAll(Integer userId, Integer couponId) {
        return queryList(userId, couponId, CouponUserConstant.STATUS_USABLE, null, null, "add_time", "desc");
    }

    public List<AswxmallCouponUser> queryAll(Integer userId) {
        return queryList(userId, null, CouponUserConstant.STATUS_USABLE, null, null, "add_time", "desc");
    }

    public AswxmallCouponUser queryOne(Integer userId, Integer couponId) {
        List<AswxmallCouponUser> couponUserList = queryList(userId, couponId, CouponUserConstant.STATUS_USABLE, 1, 1, "add_time", "desc");
    ...
    public AswxmallCouponUser findById(Integer id) {
        return couponUserMapper.selectByPrimaryKey(id);
    }


    public int update(AswxmallCouponUser couponUser) {
        couponUser.setUpdateTime(LocalDateTime.now());
        return couponUserMapper.updateByPrimaryKeySelective(couponUser);
    }

    public List<AswxmallCouponUser> queryExpired() {
        AswxmallCouponUserExample example = new AswxmallCouponUserExample();
