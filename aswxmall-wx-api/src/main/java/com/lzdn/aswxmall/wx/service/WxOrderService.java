...
import com.lzdn.aswxmall.core.express.dao.ExpressInfo;
...
import com.lzdn.aswxmall.core.system.SystemConfig;
import com.lzdn.aswxmall.core.util.DateTimeUtil;
...
import com.lzdn.aswxmall.db.util.OrderHandleOption;
...
public class WxOrderService {
    ...
    @Autowired
    private AswxmallOrderService orderService;
    @Autowired
    private AswxmallOrderGoodsService orderGoodsService;
    @Autowired
    ...
    @Autowired
    private AswxmallCommentService commentService;
    @Autowired
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<AswxmallOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (AswxmallOrder o : orderList) {
            ...
            AswxmallGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<AswxmallOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
            List<Map<String, Object>> orderGoodsVoList = new ArrayList<>(orderGoodsList.size());
            for (AswxmallOrderGoods orderGoods : orderGoodsList) {
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        List<AswxmallOrderGoods> orderGoodsList = orderGoodsService.queryByOid(order.getId());
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            AswxmallGrouponRules rules = grouponRulesService.queryById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
        ...
        // 收货地址
        AswxmallAddress checkedAddress = addressService.query(userId, addressId);
        ...
        for (AswxmallCart checkGoods : checkedGoodsList) {
            ...
            goodsids.add(checkGoods.getGoodsId().toString());
        }
        ...
        if (couponId != 0 && couponId != -1) {
            AswxmallCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, checkedGoodsPrice);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        // 添加订单商品表项
        for (AswxmallCart cartGoods : checkedGoodsList) {
            // 订单商品
            AswxmallOrderGoods orderGoods = new AswxmallOrderGoods();
        ...
        for (AswxmallCart checkGoods : checkedGoodsList) {
            ...
            AswxmallGoodsProduct product = productService.findById(productId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            AswxmallGroupon groupon = new AswxmallGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                AswxmallGroupon baseGroupon = grouponService.queryById(grouponLinkId);
    ...
    public Object cancel(Integer userId, String body) {
        ...
        // 商品货品数量增加
        List<AswxmallOrderGoods> orderGoodsList = orderGoodsService.queryByOid(orderId);
        for (AswxmallOrderGoods orderGoods : orderGoodsList) {
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        AswxmallOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        //  支付成功，有团购信息，更新团购信息
        AswxmallGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            AswxmallGrouponRules grouponRules = grouponRulesService.queryById(groupon.getRulesId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<AswxmallOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        AswxmallOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        AswxmallOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        AswxmallComment comment = new AswxmallComment();
