...
权限控制在数据库层面涉及到三个表`aswxmall_admin`, `aswxmall_role`和`aswxmall_permission`：
* aswxmall_admin表中存在roleId字段，保存角色ID数组；
* aswxmall_role表记录角色名称和角色介绍；
* aswxmall_permission表记录角色所用于的权限值。
...
后端权限基于shiro来实现，相关代码见aswxmall-admin-api模块。
...
public class AdminAdController {
    ...
    public Object list(String name, String content,
        ...
        List<AswxmallAd> adList = adService.querySelective(name, content, page, limit, sort, order);
...
aswxmall-admin模块的代码基于[vue-element-admin](https://github.com/PanJiaChen/vue-element-admin)
