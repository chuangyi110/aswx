...
import com.lzdn.aswxmall.admin.service.LogHelper;
...
public class AdminAdminController {
    ...
    public Object list(String username,
        ...
        List<AswxmallAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(AswxmallAdmin admin) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody AswxmallAdmin admin) {
        ...
        List<AswxmallAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    public Object read(@NotNull Integer id) {
        AswxmallAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody AswxmallAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody AswxmallAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        AswxmallAdmin currentAdmin = (AswxmallAdmin) currentUser.getPrincipal();
